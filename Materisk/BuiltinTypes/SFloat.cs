...
public class SFloat : SValue {
    ...
    public override SValue Add(SValue other) {
        if (other is not SFloat otherInt) throw new Exception("Can not perform Add on SFloat and " + other.BuiltinName);
        return new SFloat(Value + otherInt.Value);
    }

    public override SValue Sub(SValue other) {
        if (other is not SFloat otherInt) throw new Exception("Can not perform Sub on SFloat and " + other.BuiltinName);
        return new SFloat(Value - otherInt.Value);
    }

    public override SValue Mul(SValue other) {
        if (other is not SFloat otherInt) throw new Exception("Can not perform Mul on SFloat and " + other.BuiltinName);
        return new SFloat(Value * otherInt.Value);
    }

    public override SValue Div(SValue other) {
        if (other is not SFloat otherInt) throw new Exception("Can not perform Div on SFloat and " + other.BuiltinName);
        return new SFloat(Value / otherInt.Value);
    }

    public override SValue Mod(SValue other) {
        if (other is not SFloat otherInt) throw new Exception("Can not perform Mod on SFloat and " + other.BuiltinName);
        return new SFloat(Value % otherInt.Value);
    }

    public override SValue Equals(SValue other) {
        if (other is not SFloat otherInt) throw new Exception("Can not perform EqualsCheck on SFloat and " + other.BuiltinName);
    ...
    public override SValue CastToBuiltin(SBuiltinType other) {
        switch (other) {
            ...
            case SBuiltinType.Float:
                return new SFloat(Value);
            default: throw CastInvalid("native " + other);
        }
    }
