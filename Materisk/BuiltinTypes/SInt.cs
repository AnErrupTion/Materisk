...
public class SInt : SValue
{
    ...
    public override SValue Add(SValue other) {
        if (other is not SInt otherInt) throw new Exception("Can not perform Add on SInt and " + other.BuiltinName);

        return new SInt(Value + otherInt.Value);
    }

    public override SValue Sub(SValue other) {
        if (other is not SInt otherInt) throw new Exception("Can not perform Sub on SInt and " + other.BuiltinName);
        return new SInt(Value - otherInt.Value);
    }

    public override SValue Mul(SValue other) {
        if (other is not SInt otherInt) throw new Exception("Can not perform Mul on SInt and " + other.BuiltinName);
        return new SInt(Value * otherInt.Value);
    }

    public override SValue Div(SValue other) {
        if (other is not SInt otherInt) throw new Exception("Can not perform Div on SInt and " + other.BuiltinName);
        return new SInt(Value / otherInt.Value);
    }

    public override SValue Mod(SValue other) {
        if (other is not SInt otherInt) throw new Exception("Can not perform Mod on SInt and " + other.BuiltinName);
        return new SInt(Value % otherInt.Value);
    }
